...
import FlipXIcon from '@public/arrows-x.svg';
...
export default function initFabricHandlers() {
  ...
  const imgFlipX = document.createElement('img');
  imgFlipX.src = FlipXIcon.src;
  ...
  function renderFlipXIcon(
    ...
    ctx.translate(left, top);
    ctx.rotate(fabric.util.degreesToRadians(fabricObject.angle as number));
    ctx.drawImage(imgFlipX, -size / 2, -size / 2, size, size);
    ctx.restore();
  }

  function renderFlipYIcon(
    ctx: any,
    left: number,
    top: number,
    styleOverride: any,
    ...
    ctx.save();
    ctx.translate(left, top);
    ctx.rotate(fabric.util.degreesToRadians((fabricObject.angle as number) + 90));
    ctx.drawImage(imgFlipX, -size / 2, -size / 2, size, size);
    ctx.restore();
  }
  ...
  fabric.Image.prototype.controls.flipXControl = new fabric.Control({
    x: 0.5,
    y: -0.5,
    offsetY: 50,
    offsetX: -10,
    cursorStyle: 'pointer',
    mouseUpHandler: flipY,
    render: renderFlipYIcon
  });
  fabric.Image.prototype.controls.flipYControl = new fabric.Control({
    x: 0.5,
    y: -0.5,
    offsetY: 10,
    offsetX: -50,
    cursorStyle: 'pointer',
    mouseUpHandler: flipX,
    render: renderFlipXIcon
  });
  ...
  function flipX(eventData: any, transform: any) {
    var target = transform.target;
    var canvas = target.canvas;
    target.toggle('flipX');
    canvas.requestRenderAll();
  ...
  function flipY(eventData: any, transform: any) {
    var target = transform.target;
    var canvas = target.canvas;
    target.toggle('flipY');
    canvas.requestRenderAll();
